...
import { checkoutStripePayment } from "@/services/checkoutService";
import {
  ErrorMessage,
  Field,
  Form,
...
const Checkout = () => {
  ...
  console.log("selectedAddress", _.isEmpty(selectedAddress));
  const isLoading = useSelector(
    (state: any) => state.commonReducer.UIGlobalLoader
  );

  const checkoutProducts = useSelector(
    (state: any) => state.checkoutReducer.checkoutProducts
  );
  ...
  const handleCheckout = async () => {
    if (_.isEmpty(selectedAddress)) {
      toast.error("Please select your address for shipping.");
    } else {
      const reqBody = {
        products: checkoutProducts,
        address: selectedAddress,
      };
      console.log("checkout--===->>>", reqBody);
      const response: any = await checkoutStripePayment(reqBody);
  ...
  const totalCartPrice = () => {
    const totalPrice1 = checkoutProducts.reduce(
              ...
              <div className="mt-10 lg:mt-0">
                ...
                <div className="mt-4 bg-white border border-gray-200 rounded-lg shadow-sm">
                  ...
                  <ul role="list" className="divide-y divide-gray-200">
                    {_.map(checkoutProducts, (product) => (
                      <>
